...
description: This is the API documentation for the OpenBao Consul secrets engine.
---
...
see the [OpenBao Consul documentation](/bao/docs/secrets/consul).
...
- `partition` `(string: "")` - Specifies the Consul admin partition in which the token is generated.
  ...
  OpenBao role must also exist inside the given partition. If not provided, the partition `default`
  is used.
...
- `consul_namespace` `(string: "")` - Specifies the Consul namespace in which the token is generated.
  The namespace must exist, and the Consul policies or roles assigned to the OpenBao role must also exist
...
- `ttl` `(duration: "")` – Specifies the TTL for this role. If not
  provided, the default OpenBao TTL is used. Uses [duration format strings](/bao/docs/concepts/duration-format).

- `max_ttl` `(duration: "")` – Specifies the max TTL for this role. If not
  provided, the default OpenBao Max TTL is used. Uses [duration format strings](/bao/docs/concepts/duration-format).
...
- `lease` <sup>DEPRECATED (1.11)</sup> `(string: "")` – Specifies the lease for this role.
  Uses [duration format strings](/bao/docs/concepts/duration-format). If not
  provided, the default OpenBao lease is used.
